const tmdbPopular = "https://api.themoviedb.org/3/tv/popular",
  tmdbDiscover = "https://api.themoviedb.org/3/discover/tv",
...
let currentGenreId = null,
  ...
  isSearching = !1,
  currentYear = null;
const videasyLink = "https://player.videasy.net/tv/",
  vidgeLink = "https://vidbinge.dev/embed/tv/",
  oneoneoneLink = "https://111movies.com/tv/",
  vidfastLink = "https://vidfast.pro/tv/",
  vidsrcLink = "https://vidsrc.xyz/embed/tv/",
  superembedLink = "https://multiembed.mov/directstream.php?video_id=",
  vidlinkLink = "https://vidlink.pro/tv/",
  moviesapiLink = "https://moviesapi.club/tv/",
  twoembedLink = "https://2embed.cc/embedtv/",
  vidvipLink = "https://vidsrc.vip/embed/tv/";
let season = 1,
  episode = 1,
  inputSeason = document.getElementById("inputSeason"),
  inputEpisode = document.getElementById("inputEpisode");
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function t(e, t) {
    ...
    const n = `https://api.themoviedb.org/3/search/tv?api_key=${tmdbApi}&query=${encodeURIComponent(
      e
    )}&page=${t}`;
    (currentYear = null),
      (currentGenreId = null),
  ...
  inputBtn.addEventListener("click", () => {
    (season = inputSeason.value),
      ...
      "green" === document.getElementById("videasyBtn").style.backgroundColor
        ? e(videasyLink, `${n}/${season}/${episode}`, videasyBtn)
        : "green" === document.getElementById("vidgeBtn").style.backgroundColor
        ? e(vidgeLink, `${n}/${season}/${episode}`, vidgeBtn)
        : "green" ===
          document.getElementById("oneoneoneBtn").style.backgroundColor
        ? e(oneoneoneLink, `${n}/${season}/${episode}`, oneoneoneBtn)
        : "green" ===
          document.getElementById("vidfastBtn").style.backgroundColor
        ? e(vidfastLink, `${n}/${season}/${episode}`, vidfastBtn)
        : "green" === document.getElementById("vidsrcBtn").style.backgroundColor
        ? e(vidsrcLink, `${n}/${season}-${episode}`, vidsrcBtn)
        : "green" ===
          document.getElementById("superembedBtn").style.backgroundColor
        ? e(
            superembedLink,
            `${n}&tmdb=1&s=${season}&e=${episode}`,
            superembedBtn
          )
        : "green" ===
          document.getElementById("vidlinkBtn").style.backgroundColor
        ? e(vidlinkLink, `${n}/${season}/${episode}`, vidlinkBtn)
        : "green" ===
          document.getElementById("moviesapiBtn").style.backgroundColor
        ? e(moviesapiLink, `${n}/${season}/${episode}`, moviesapiBtn)
        : "green" ===
          document.getElementById("twoembedBtn").style.backgroundColor
        ? e(twoembedLink, `${n}&s=${season}&e=${episode}`, twoembedBtn)
        : "green" ===
            document.getElementById("vidvipBtn").style.backgroundColor &&
          e(vidvipLink, `${n}/${season}/${episode}`, vidvipBtn),
    ...
    vidsrcBtn.addEventListener("click", () => {
      myIframe.removeAttribute("sandbox"),
        e(vidsrcLink, `${n}/${season}-${episode}`, vidsrcBtn),
  ...
  function o(t) {
    ...
    o.forEach((o, d) => {
      ...
      d.forEach((e, n) => {
        if (t.results && t.results[n] && t.results[n].name) {
          const o = t.results[n].name,
            d = t.results[n].first_air_date
              ? t.results[n].first_air_date.substring(0, 4)
              : "N/A";
          e.textContent = `${o} (${d})`;
  ...
  function r() {
    !(function (e, t, n) {
      ...
      const l = document.getElementById("pagination-container"),
        a = document.getElementById("previous-page"),
        m = document.getElementById("next-page");
      if (
        (a.classList.remove("disabled"),
        1 === e && a.classList.add("disabled"),
      ...
      m.classList.remove("disabled"),
        e === t && m.classList.add("disabled"),
        (m.querySelector("a").onclick = function () {
          return e < t && (e++, r()), !1;
        }),
        (l.innerHTML = a.outerHTML + o + m.outerHTML);
      ...
      isSearching
        ...
        : (function (e, t = currentGenreId, n = currentYear) {
            ...
            (isSearching = !1),
              n
                ? ((d = `${tmdbDiscover}?api_key=${tmdbApi}&first_air_date_year=${n}&page=${e}`),
                  (currentGenreId = null))
                : t
                ? ((d = `${tmdbDiscover}?api_key=${tmdbApi}&with_genres=${t}&page=${e}`),
                  (currentYear = null))
                : ((d = `${tmdbDiscover}?api_key=${tmdbApi}&first_air_date_year=2025&page=${e}`),
                  (currentYear = null)),
              fetch(d)
                .then((e) => e.json())
                .then((e) => {
      ...
      .then((e) => {
        ...
        r.forEach((t, n) => {
          e.results &&
            e.results[n] &&
            e.results[n].name &&
            (t.textContent = e.results[n].name);
        }),
          l.forEach((t, n) => {
            e.results &&
              e.results[n] &&
          ...
          a.forEach((t, n) => {
            e.results &&
              e.results[n] &&
              e.results[n].first_air_date &&
              (t.textContent = e.results[n].first_air_date);
          });
      })
      .catch((e) => console.log(e));
